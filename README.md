...
- 2026-09-22: Removed alternate-manager lockfile import and hardened refusal paths — `bpm import` now accepts only npm `package-lock.json` v2/v3 (content-based, so renamed copies still import; yarn/pnpm/bun lockfiles fail cleanly with no output side effects because their physical placement cannot be represented safely); `bpm install --frozen`/`bpm ci` now refuse when `package.json` is missing, unreadable, or malformed instead of warning and skipping the drift check (lock-only projects still install without `--frozen`); `install.sh` validates `BPM_VERSION` against a strict `[v]MAJOR.MINOR.PATCH[-prerelease]` grammar before any tool runs, requires the `--registry`/`--global` capability tokens and an exact `bpm --version` identity match for explicit pins, and refuses source-build fallback for a pinned release (latest mode keeps the fallback); `bpm doctor` replaced stale "not yet supported" messages with neutral guidance and renamed `WORKSPACES_UNSUPPORTED` to `WORKSPACES_DECLARED` (info); package-image index decoding is now buffered with an error-equivalence regression against the unbuffered decoder.
- 2026-09-08: Added adaptive download concurrency — live-registry measurements showed npmjs shaping per-client concurrent load (per-request latency 153ms at 4 concurrent downloads but 1245ms at 24, with aggregate goodput saturated, so aggressive fan-out made installs 2-3s slower); artifact downloads now start at full width and halve multiplicatively when the latency EWMA balloons, with slow upward re-probes, converging within ~1s and holding full width on unshaped networks (deterministic replay unchanged, shaped-network installs 4.3-5.0s → 1.6-2.1s); the async resolver client now also honors `BPM_HTTP2` like the artifact client.
- 2026-08-29: Cut deterministic cold-install wall 1638→1300ms on a replay registry bun and pnpm also install from (identical bytes, latency, and bandwidth) — packument prefetch lookahead deepened 2→3 levels and artifact download hints now propagate through descendant prefetch so a package's tarball starts transferring the moment its selected version is known instead of when the DFS places it (deep-chain platform binaries like `@esbuild/darwin-arm64` previously began a ~465ms transfer after the spine finished; speculative downloads rose ~5%, bounded by the 256-entry lookahead budget; `BPM_ALL_HINTS=0`/`BPM_ROOT_LOOKAHEAD=2` restore the old behavior); on the identical-network head-to-head bpm now beats pnpm (~1300 vs ~1340ms) and the gap to bun narrows to ~285ms.
- 2026-08-29: Made plain `bpm install` npm-compatible after manual package.json edits — a drifted bpm.lock now re-resolves (adding new dependencies and pruning removed ones) instead of silently replaying the stale lock, with fresh resolves reading the prior plan so stale entries reconcile, empty graphs validating as plan-cache hits, and frozen/package-lock-only flows unchanged; the drift check compares merged root declarations (deps+dev+optional+peer) and overrides, so hand-authored and imported locks with a different dev/prod split are unaffected.
- 2026-08-29: Halved the true_cold gap to bun (real-registry median 1559→1410ms; replay-with-realistic-latency wall 1590→1481ms) — root-warmup packument fetches now run detached instead of behind a joined barrier so one slow packument can no longer gate the resolve loop, and the resolver's default in-flight request ceiling rose 32→64 (`BPM_RESOLVER_MAX_IN_FLIGHT` clamp 128); both proven ≥2σ on a new replay profile calibrated to live npmjs latency and bandwidth (the harness gained `--bandwidth-mbps` throttling), where the earlier 20ms/loopback profile had correctly shown no win for concurrency changes with nothing to hide.
- 2026-08-28: Made the relay view the default project attachment on Unix — `node_modules` top-level entries symlink into the immutable graph volume, cutting `project_attachment` 104–121→~4ms (cold replay wall 652→~569ms same-day; reattach-after-clear 121→4ms; real-registry `true_cold` median 1687→1559ms) — with the write-isolation trade pnpm's virtual store and bun's hardlink mode make; Next.js/Turbopack-style projects auto-keep the isolated local view via `BPM_LOCAL_VIEW_PACKAGES`, non-Unix platforms always get an isolated view, `BPM_PROJECT_VIEW=relay|local|reflink` overrides, plan validation accepts relay targets exactly, and existing isolated views migrate on the next attach.
- 2026-08-27: Cut cold-install wall time on the deterministic replay harness 686→554ms — the install lease (publication walk, access rows, lease rows) now runs concurrently with volume build/direct materialization under GC's 30-day freshness grace (joining before graph record keeps success-without-protection impossible), and tar extraction skips the follow-up chmod whenever the archive mode provably equals the umask-produced create mode (one fstat per extraction, one syscall saved per file on ~20k-entry packages); real-registry `true_cold` median improved 1837→1687ms.
