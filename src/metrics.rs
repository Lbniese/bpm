...
pub struct Counters {
    ...
    /// Milliseconds of materialization overlap with the download pipeline tail.
    /// High-water mark: tracks the maximum observed overlap duration.
    pub materialization_overlap_with_download_ms: u64,
}
...
impl Counters {
    ...
    pub fn merge(&mut self, other: &Counters) {
        ...
        self.materialization_overlap_with_download_ms = self
            .materialization_overlap_with_download_ms
            .max(other.materialization_overlap_with_download_ms);
    }
}
...
impl Metrics {
    ...
    /// Record the overlap duration (ms) between materialization and the
    /// download pipeline tail. Uses high-water mark semantics so the
    /// maximum observed overlap is retained.
    pub fn add_overlap(&mut self, ms: u64) {
        self.counters.materialization_overlap_with_download_ms = self
            .counters
            .materialization_overlap_with_download_ms
            .max(ms);
    }
    ...
    pub fn to_json(&self) -> String {
        ...
        counters.insert(
            "materialization_overlap_with_download_ms".into(),
            serde_json::Value::from(self.counters.materialization_overlap_with_download_ms),
        );
