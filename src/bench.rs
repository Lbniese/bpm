...
pub fn run_scenario_with_runner(
    ...
    for run_index in 0..num_runs {
        ...
        // When profiling parity, redirect every tool through the counting
        // proxy via a per-work-dir `.npmrc` (npm/pnpm/bpm all read cwd's .npmrc).
        if let Some(addr) = parity_addr {
            fs::write(
                work_dir.join(".npmrc"),
                crate::parity_proxy::parity_npmrc_content(addr),
            )?;
        }
...
fn seed_install(
    ...
    let command = install_command_spec(tool, work_dir, bpm_store, scenario, None, ignore_scripts);
    let outcome = runner.run(&command)?;
    if outcome.exit_code != 0 {
        anyhow::bail!(
...
pub struct RunSuiteOptions {
    ...
    /// Route every tool through the counting parity proxy and capture a
    /// per-tool `NetworkShape`. Default off: the proxy normalizes
    /// transport to HTTP/1.1, so it measures network *shape*, not production
    /// wall-clock.
    ...
    F: FnMut(Tool) -> bool,
{
    ...
    for &scenario in scenarios {
        ...
        // The parity proxy spans all tools for this scenario (one process per
        // scenario); each tool's network shape is snapshotted between tools.
        ...
        for &tool in &available_tools {
            ...
            if let Some(proxy) = &parity_proxy {
                proxy.clear();
            }
            let mut tool_result = run_scenario(
                scenario,
                fixture,
                tool,
                options.num_runs,
            ...
            if let Some(proxy) = &parity_proxy {
                let records = proxy.snapshot_and_clear();
                if !records.is_empty() {
                    tool_result.network =
                        Some(crate::parity_proxy::compute_network_shape(&records));
                }
            }
