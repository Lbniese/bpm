...
//!
//! `doctor` can only inspect manifests; it cannot install, resolve,
//! fetch, or run scripts. `doctor` reports what it observes and exits nonzero when an
...
/// only fields `doctor` can describe without resolving or installing.
