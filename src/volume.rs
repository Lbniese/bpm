...
pub fn attach_project_local_with_backend(
    ...
    // Parallel tree linking: each entry's tree is independent.
    ...
    let mut owned: Vec<ManagedEntry> = if workers <= 1 {
        ...
        for entry in &entries {
            ...
            if matches!(backend, MaterializeBackend::Reflink) {
                ...
                crate::materializer::copy_tree(&source, &target)
        ...
        let proj_nm = std::sync::Arc::new(proj_nm);
        let identities = std::sync::Arc::new(identities);
        std::thread::scope(|s| {
            let mut handles = Vec::with_capacity(workers);
            for chunk in entries.chunks(chunk) {
                let proj_nm = proj_nm.clone();
                let identities = identities.clone();
                handles.push(s.spawn(move || {
                    let mut local_owned = Vec::with_capacity(chunk.len());
                    let mut local_stats = AttachStats::default();
                    for entry in chunk {
                        let source = entry.path();
                        let name = entry.file_name();
                        let name_str = name.to_string_lossy().into_owned();
                        ...
                        local_owned.push(ManagedEntry {
                            path: format!("node_modules/{name_str}"),
                            mode: mode.to_string(),
                            identity: identity.clone(),
                        });
                    }
                    Ok::<_, VolumeError>((local_stats, local_owned))
                }));
            }
            let mut out = Vec::with_capacity(entries.len());
            for handle in handles {
                match handle.join() {
                    Ok(Ok((local_stats, mut local_owned))) => {
                        stats.relays_created += local_stats.relays_created;
                        out.append(&mut local_owned);
                    }
                    Ok(Err(err)) => return Err(err),
                    Err(panic) => std::panic::resume_unwind(panic),
                }
            }
            Ok::<_, VolumeError>(out)
        })?
    };
    // Sort by path to guarantee deterministic output regardless of
    // thread scheduling (mirrors compute_entry_identities).
    owned.sort_by(|a, b| a.path.cmp(&b.path));
...
mod ownership_tests {
    ...
    #[test]
    fn parallel_project_attachment_is_identical_to_serial() {
        let volume_root = tempdir().unwrap();
        make_volume(volume_root.path(), &[("foo", r#"{"name":"foo"}"#)]);
        let vol_ref = volume_ref(volume_root.path());

        // Attach into project_a (serial path, workers=1).
        let project_a = tempdir().unwrap();
        let outcome_a = attach_project_local_with_backend(
            project_a.path(),
            &vol_ref,
            MaterializeBackend::Reflink,
        )
        ...
        // Attach into project_b (parallel path, workers>1).
        let project_b = tempdir().unwrap();
        let outcome_b = attach_project_local_with_backend(
            project_b.path(),
            &vol_ref,
            MaterializeBackend::Reflink,
        )
        ...
        // Both outcomes must produce the same ownership set.
        assert_eq!(outcome_a.owned.len(), outcome_b.owned.len());
        for (a, b) in outcome_a.owned.iter().zip(outcome_b.owned.iter()) {
            assert_eq!(a.path, b.path);
            assert_eq!(a.identity, b.identity);
        }

        // The actual node_modules/foo directory content must match.
        let foo_a = project_a.path().join("node_modules/foo/package.json");
        let foo_b = project_b.path().join("node_modules/foo/package.json");
        assert_eq!(fs::read(&foo_a).unwrap(), fs::read(&foo_b).unwrap());
    }
}
