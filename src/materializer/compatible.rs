//! Compatible-mode (npm v3 hoisted layout) project-local materialization.
//!
//! **Note (S4.1.1):** this module is an extraction of the deterministic
//! compatible-mode projection currently implemented in `src/materializer.rs`.
//! It is not yet wired into the crate module tree (no `mod compatible;` /
//! `src/materializer/mod.rs` exists yet) — that wiring, plus the sibling
//! `strict.rs` mode and removal of the flat `src/materializer.rs` file, are
//! separate follow-on tasks (S4.1.2-S4.1.4). Until that wiring lands, this
//! file is inert (not part of the compiled crate) and `src/materializer.rs`
//! remains the active, in-tree implementation.
//!
