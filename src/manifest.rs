...
/// `peerDependenciesMeta` entry. Only `optional` is tracked.
#[derive(Debug, Clone, Deserialize, PartialEq, Eq)]
pub struct PeerMeta {
    #[serde(default)]
    pub optional: bool,
...
/// npm's rules; exotic historical names are out of scope and will be reported
/// as diagnostics by `bpm doctor`.
