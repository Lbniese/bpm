...
// === Additional parity coverage tests ===
...
// === full-corpus parity tests using a multi-package mock ===
...
// === resolver-mode/peer-option matrix regressions ===

/// regression: a required peer absent from the tree must fail strict
...
/// regression: the non-streaming kill-switch matrix must print a
...
// === registry trust-boundary regression tests ===
...
/// Forced-overflow parity for the async+streaming pipeline. With
...
// ── concurrency-stress parity (wide sibling fan-out) ─────────────
...
/// Concurrency-stress parity test: a root with a wide
...
/// Determinism-under-reordering test: run the async resolve twice
