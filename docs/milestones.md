...
| 7 — Cold-path performance | representative benchmark corpus, persistent metadata efficiency, native-resolution profiling, derived lifecycle decision | ✅ **Done** (Plan 005). The dual-resolver architecture (blocking + async) has been unified into a single I/O-agnostic placement core (`src/resolver/placement.rs` + `src/resolver/fetch.rs`), and the async resolver is now the default (Plan 005, Phases 2–5). The parity corpus (Plan 002) confirms byte-identical output between blocking and async paths for all cases. The cold-resolver remains the primary bottleneck (bpm ~6.8× slower than pnpm on `true_cold` `large-frontend` at the `21b87fc` baseline); resolver unification is a structural prerequisite for further optimization, not a speedup in itself. Async resolution is default-on; `BPM_ASYNC_RESOLVE=0` is a kill-switch. Derived-artifact integration deferred by decision — it is a warm/incremental-path optimization (the derived store is empty on a cold install, so it yields zero cold-path benefit); tracked as a follow-up, not an M7 cold-path item. |
